...
'''Constraint Satisfaction Routines
   A) class Variable
    ...
    B) class constraint
      ...
      This class allows one to define constraints specified by variable
      scopes and predicate functions. The predicate returns True if the
      values for the variables in scope are valid assignments, and False
      otherwise. This costs more memory than the table of satisfying
      assignments method provided in the A2 starter code, but is unable
      to support GAC.

      On initialization the variables the constraint is over is
      specified (i.e. the scope of the constraint), as well as the predicate
      function that takes the elements of the scope as arguments in order.

      Because the constraints are implemented as predicates, no satisfying
      tuples must be added.
...
class Variable: 
    ...
    def assign(self, value):
        ...
        if self.is_assigned() or not self.in_cur_domain(value):
            ...
            raise Exception("Trying to assign variable {} that is already assigned or illegal value (not in curdom)".format(self))
            return
...
class Constraint: 
    ...
    def __init__(self, name, scope, pred): 
        '''create a constraint object, specify the constraint name (a
        string), its scope (an ORDERED list of variable objects), and
        the predicate function over its scope.
        The order of the variables in the scope is the same as their order
        in the predicate, and is critical to the functioning of the constraint.
        '''
    ...
    # Removing for predicates over tables
    #
    ...
    def check(self, vals):
        ...
        # List of vals must be the same size as predicate scope
        if len(vals) != len(self.get_scope):
            raise Exception('Constraint check arity mismatch')
        return self.pred(*vals)
    ...
    # TODO: Confirm that this can be safely removed.
    #
...
class BT:
    ...
    def bt_search(self,propagator):
        ...
        self.unasgn_vars = []
        print('BT sees vars: {}'.format(self.csp.vars))
        print('BT sees vals: {}'.format(list(map(lambda v: v.get_assigned_value(), self.csp.vars))))
        print('BT sees domains: {}'.format(list(map(lambda v: v.cur_domain(), self.csp.vars))))
